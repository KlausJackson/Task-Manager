...
async function login(req, res) {
	try {
		...
		if (!user) return returnStatus(401, 'Unable to login. Invalid credentials.', null, null, res)
		const token = await user.generateToken()
...
async function deleteMe(req, res) {
	try {
		...
		if (!isMatch) return returnStatus(401, 'Unable to delete account. Invalid credentials.', null, null, res)
